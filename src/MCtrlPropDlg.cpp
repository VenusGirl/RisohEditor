// MCtrlPropDlg.cpp --- "Properties for Control" Dialog
//////////////////////////////////////////////////////////////////////////////
...
DWORD MCtrlPropDlg::GetItemAndFlags(DialogItem& item)
{
	DWORD flags = m_flags;
	...
	if (!strStyle.empty())
		flags |= F_STYLE;
	item.m_style = mstr_parse_int(strStyle.c_str(), false);

	MString strExStyle = GetDlgItemText(edt7);
	mstr_trim(strExStyle);
	if (!strExStyle.empty())
		flags |= F_EXSTYLE;
	item.m_ex_style = mstr_parse_int(strExStyle.c_str(), false);
...
BOOL MCtrlPropDlg::SetInfo(DWORD flags)
{
	if (m_indeces.empty())
		return TRUE;
...
void MCtrlPropDlg::InitTables(LPCTSTR pszClass)
{
	m_style_table.clear();
...
void MCtrlPropDlg::ApplySelection(HWND hLst, std::vector<BYTE>& sel)
{
	m_bUpdating = TRUE;
...
void
MCtrlPropDlg::ApplySelection(HWND hLst, ConstantsDB::TableType& table,
...
BOOL MCtrlPropDlg::OnInitDialog(HWND hwnd, HWND hwndFocus, LPARAM lParam)
{
...
void MCtrlPropDlg::OnOK(HWND hwnd)
{
...
void MCtrlPropDlg::OnLst1(HWND hwnd)
{
	if (m_bUpdating)
		return;
...
void MCtrlPropDlg::OnLst2(HWND hwnd)
{
	if (m_bUpdating)
		return;
...
void MCtrlPropDlg::OnEdt6(HWND hwnd)
{
	if (m_bUpdating)
		return;
...
void MCtrlPropDlg::OnEdt7(HWND hwnd)
{
	if (m_bUpdating)
		return;
...
void MCtrlPropDlg::UpdateClass(HWND hwnd, HWND hLst1, const MString& strClass)
{
	MString strSuper;
...
void MCtrlPropDlg::OnPsh1(HWND hwnd)
{
	MCtrlDataDlg dialog(m_item.m_extra);
	dialog.DialogBoxDx(hwnd);
}

void MCtrlPropDlg::OnPsh2(HWND hwnd)
{
...
void MCtrlPropDlg::OnPsh3(HWND hwnd)
{
...
void MCtrlPropDlg::OnCommand(HWND hwnd, int id, HWND hwndCtl, UINT codeNotify)
{
	HWND hLst1 = GetDlgItem(hwnd, lst1);
	HWND hCmb1 = GetDlgItem(hwnd, cmb1);
	HWND hCmb4 = GetDlgItem(hwnd, cmb4);
...
LRESULT MCtrlPropDlg::OnNotify(HWND hwnd, int idFrom, LPNMHDR pnmhdr)
{
...
INT_PTR CALLBACK
MCtrlPropDlg::DialogProcDx(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch (uMsg)
	{
